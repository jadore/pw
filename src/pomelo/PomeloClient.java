...
public class PomeloClient {
	...
	public void init() {
		socket.connect(new IOCallback() {
			public void onConnect() {
				Logger.i("pomeloclient is connected.");
				SharedPreferences sharedPre = MyApplication.getInstance().getSharedPreferences(
						IPolemoService.TAG, Context.MODE_PRIVATE);
				sharedPre.edit().putBoolean(IPolemoService.PREF_CONNECTED, true).commit();	
			}

			public void onMessage(JSONObject json, IOAcknowledge ack) {
				Logger.i("pomelo send message of string.");
			}
	...
	private void emit(String route, JSONObject message) {
		List<DataListener> list = listeners.get(route);
		if (list == null) {
			Logger.i("there is no listeners.");
			return;
		}
